...
def run_swiglu(
    ...
    swiglu = ffn(d_model=d_model, d_ff=d_ff, device=in_features.device, dtype=in_features.dtype)
    with torch.no_grad():
        swiglu.up.W.copy_(w1_weight)
        swiglu.down.W.copy_(w2_weight)
        swiglu.gate.W.copy_(w3_weight)
...
def run_multihead_self_attention(
    ...
    with torch.no_grad():
        ...
        mha.output_proj.W.copy_(o_proj_weight)
...
def run_transformer_block(
    ...
    with torch.no_grad():
        block.attn.q_proj.W.copy_(weights["attn.q_proj.weight"])
        block.attn.k_proj.W.copy_(weights["attn.k_proj.weight"])
        block.attn.v_proj.W.copy_(weights["attn.v_proj.weight"])
        block.attn.output_proj.W.copy_(weights["attn.output_proj.weight"])

        block.ln1.weight.copy_(weights["ln1.weight"])
        block.ln2.weight.copy_(weights["ln2.weight"])

        block.ffn.up.W.copy_(weights["ffn.w1.weight"])
        block.ffn.down.W.copy_(weights["ffn.w2.weight"])
        block.ffn.gate.W.copy_(weights["ffn.w3.weight"])
...
def run_transformer_lm(
    ...
    with torch.no_grad():
        ...
        for layer_idx in range(num_layers):
            ...
            layer.attn.q_proj.W.copy_(weights[f"{prefix}.attn.q_proj.weight"])
            layer.attn.k_proj.W.copy_(weights[f"{prefix}.attn.k_proj.weight"])
            layer.attn.v_proj.W.copy_(weights[f"{prefix}.attn.v_proj.weight"])
            layer.attn.output_proj.W.copy_(weights[f"{prefix}.attn.output_proj.weight"])
            layer.ln1.weight.copy_(weights[f"{prefix}.ln1.weight"])
            layer.ln2.weight.copy_(weights[f"{prefix}.ln2.weight"])
            layer.ffn.up.W.copy_(weights[f"{prefix}.ffn.w1.weight"])
            layer.ffn.down.W.copy_(weights[f"{prefix}.ffn.w2.weight"])
            layer.ffn.gate.W.copy_(weights[f"{prefix}.ffn.w3.weight"])
...
def run_rmsnorm(
    ...
    layer = RMSNorm(d_model=d_model, eps=eps, device=in_features.device, dtype=in_features.dtype)
    with torch.no_grad():
...
def run_get_batch(
    ...
    """
    Given a dataset (a 1D numpy array of integers), a desired batch size and context length, 
